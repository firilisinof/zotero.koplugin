...
describe("Zotero background library sync", function()
    ...
    it("commits for an instance that never built a browser without building one", function()
        env:syncRoutes()
        local unbuilt = newPlugin(nil)
        unbuilt:attachSyncJob(plugin.sync_job)
        unbuilt:onZoteroSyncAction()
        runtime:work()
        assert.equals("1230", api.getLibraryVersion())
        assert.equals("Success.", runtime:last().text)
        assert.is_nil(unbuilt.browser)
        assert.same({}, refreshes)
    end)
